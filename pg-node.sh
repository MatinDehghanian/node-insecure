...
install_node() {
    ...
    FILES_URL_PREFIX="https://raw.githubusercontent.com/MatinDehghanian/node-insecure/main"
    COMPOSE_FILES_URL_PREFIX="https://raw.githubusercontent.com/MatinDehghanian/node-insecure/main"
    ...
    if [ "$AUTO_CONFIRM" = true ]; then
        ...
        read -r -p "Do you want to DISABLE TLS encryption? (Y/n) [WARNING: Not recommended for production]: " use_no_tls
    fi
    if [[ "$use_no_tls" =~ ^[Yy]$ ]]; then
        TLS_ENABLED=0
        colorized_echo red "⚠ TLS is DISABLED. Connection will NOT be encrypted!"
    else
        TLS_ENABLED=1
        if [ "$AUTO_CONFIRM" != true ]; then
    ...
    # Set TLS_ENABLED in .env
    if [ "$TLS_ENABLED" -eq 0 ]; then
        sed -i 's/^# \(TLS_ENABLED *=.*\)/TLS_ENABLED= false/' "$APP_DIR/.env"
    else
...
install_command() {
    ...
    check_version_exists() {
        local version=$1
        repo_url="https://api.github.com/repos/MatinDehghanian/node-insecure/releases"
